...
const requiredInvariants = [
  'R1_NO_DUPLICATE_SEMANTIC_WORK', 'R2_PURPOSE_LIMIT_FREE_TEXT', 'R3_DERIVATION_RETURNS_VALUE',
  'R4_NO_UNUSED_TELEMETRY', 'R5_NO_UNUSED_DERIVED_STATE', 'C1_SINGLE_RULE_SOURCE', 'C2_SAME_TERM_SAME_MEANING',
  'C3_NO_EVIDENCE_IS_NOT_EVIDENCE', 'C4_PRIVACY_CLAIMS_MATCH_DATA_FLOW', 'C5_DETECTION_IS_NOT_AUTHORITY',
  'C6_DOCS_COPY_CODE_ALIGN', 'C7_NO_SILENT_CLASSIFICATION', 'C8_CANONICAL_STATE_INTERPRETATION',
];
...
// R2: response modality can inspect answer structure, never prose semantics or unrelated metadata.
...
if (/\.length\s*[<>]=?\s*\d+/.test(interactionBody) || /reduce\([^)]*\.length/.test(interactionBody)) issue('FREE_TEXT_LENGTH_PROFILE', 'analyzeInteraction must not infer UX state from reflection-text length.');
...
if (!interactionBody.includes("pace: 'balanced'") || !interactionBody.includes('minimalism: false') || !interactionBody.includes('overload: false')) issue('UX_PROFILE_AUTHORITY_DRIFT', 'Content-derived pace/minimalism/overload must remain disabled unless separately authorized.');
...
if (!contextEngine.includes('sessionStartedAt: string') || !contextEngine.includes('lastVisitAt?: string') || !contextEngine.includes('device: DeviceClass')) issue('REQUIRED_UX_SIGNAL_MISSING', 'Minimal context contract must retain session start, last visit and device because current decisions use them.');
if (!contextLayer.includes("JSON.stringify({ lastVisitAt: new Date().toISOString() })")) issue('USAGE_STORAGE_SCOPE_DRIFT', 'Persistent UX usage storage must contain only lastVisitAt; session duration and device are session-local.');

// R5: don't carry or persist derived state with no current consumer.
for (const dead of ['export type ProcessContext', 'historyCount:', 'daysSinceLastCheckpoint:', 'profile: InteractionProfile', 'localHour:', 'weekday:']) {
  if (contextEngine.includes(dead)) issue('UNUSED_CONTEXT_DERIVATION', `Context engine still carries unused derived state: ${dead}`);
}
if (contextLayer.includes('analyzeInteraction')) issue('UNUSED_CONTEXT_PROFILE', 'Context layer must not compute a response profile that the UX governor does not consume.');
if (orgSignalLayer.includes('mati-organizational-signal-v0') || orgSignalLayer.includes('SIGNAL_SNAPSHOT_KEY')) issue('UNUSED_SIGNAL_SNAPSHOT', 'Organizational signal layer must not persist an internal signal snapshot with no reader.');
...
// R3: user-visible derivations disclose derivation and preserve source authority.
for (const phrase of ['זה תמצות מכני של מה שכתבת', 'השאירי את הטקסט המקורי כסמכות', 'זה חישוב מתוך המספרים שהזנת, לא הערכה של המערכת', 'כל המסקנות כאן נשענות על מה שהזנת']) {
  if (!page.includes(phrase)) issue('DERIVATION_DISCLOSURE_DRIFT', `User-visible derivation disclosure is missing: ${phrase}`);
}

// C1: Stage 1 has one semantic gate across rule, save and home guidance.
...
// C7: unresolved calendar gaps remain unresolved in every support layer.
...
if (!experienceShell.includes('if (!activeStage) return') || !contextLayer.includes('if (!activeStage) return null') || !workSession.includes('return state.manualStage ?? stageFromDate();')) issue('UNRESOLVED_STAGE_PATH_DRIFT', 'Support layers must explicitly preserve the between-window unresolved stage until the instructor chooses.');

// C8: every support layer uses the canonical persisted-state interpreter.
for (const [name, source] of [['ExperienceShell', experienceShell], ['ContextLayer', contextLayer], ['WorkSessionLayer', workSession], ['OrganizationalSignalLayer', orgSignalLayer]]) {
...
if (!stateHydration.includes("MATI_STATE_KEY = 'mati-v2'") || !stateHydration.includes("MATI_LEGACY_KEY = 'mati-v1'")) issue('STATE_KEY_SOURCE_DRIFT', 'Canonical state hydration must own both current and legacy storage keys.');
const legacyMappings = [['q1','evidence'],['q2','evidence'],['q3','notes'],['q4','evidence'],['q5','reflection'],['q6','culturePositiveSign'],['q7','evidence'],['q8','didNotWork'],['q9','']];
for (const [question, field] of legacyMappings) {
  const token = field ? `${question}: { ...emptyState.formative.answers.${question}, ${field}: legacyAnswers.${question}` : `${question}: { ...emptyState.formative.answers.${question} }`;
  if (!stateHydration.includes(token)) issue('LEGACY_MIGRATION_DRIFT', `Canonical state hydration is missing legacy mapping for ${question}${field ? ` -> ${field}` : ''}.`);
  if (!page.includes(token)) issue('PAGE_MIGRATION_DRIFT', `Professional page migration no longer matches canonical legacy mapping for ${question}${field ? ` -> ${field}` : ''}.`);
}

// C2/R1: cross-stage repetitions are allowed only with explicit temporal meaning.
...
for (const item of reviewed) if (item.status !== 'DISTINCT_TEMPORAL_MEANING' || !item.earlierMeaning || !item.laterMeaning) issue('CROSS_STAGE_SEMANTIC_AMBIGUITY', `${item.earlier} -> ${item.later} is not explicitly distinguished.`);
for (const label of ['מסגרת זמן גסה', 'תקופת ההערכה', 'מטרת SMART אחת', 'מטרות מרכזיות בתוכנית']) if (!page.includes(label)) issue('TEMPORAL_LABEL_DRIFT', `UI no longer distinguishes a reviewed cross-stage representation: ${label}`);

// C5: inquiry heuristics never become gates, scores or causal diagnoses.
...
for (const heuristic of heuristics) if (heuristic.authority !== 'INQUIRY_ONLY' || heuristic.mayGate !== false || heuristic.mayScore !== false || heuristic.mayAssertCausality !== false) issue('HEURISTIC_AUTHORITY_DRIFT', `${heuristic.id} exceeds inquiry-only authority.`);
...
for (const causal of ['הסיבה היא', 'נגרם בגלל', 'גורם ל', 'מוכיח ש']) if (contradictionBody.includes(causal)) issue('CAUSALITY_OVERREACH', `Contradiction detector asserts causality: ${causal}`);

// C6: docs, user copy and code describe the same current behavior.
...
// C4: privacy claims match every current network, aggregation and identity path.
for (const phrase of ['Reflection belongs to the instructor', 'no automatic cross-device collection', 'manual import of multiple packs', 'may not automatically', 'assert causality', 'pseudonymous rather than anonymous']) if (!orgContract.includes(phrase)) issue('ORGANIZATIONAL_AUTHORITY_DRIFT', `Organizational contract is missing current boundary: ${phrase}`);
...
for (const [name, source] of [['org page', orgPage], ['org console', orgConsole], ['signal preview', orgPreview]]) for (const forbidden of ['משתתפות אנונימיות', 'קוד מסגרת אנונימי']) if (source.includes(forbidden)) issue('ANONYMITY_OVERCLAIM', `${name} overstates pseudonymous data as anonymous: ${forbidden}`);
...
// C3: protected professional conflict is surfaced, not silently rewritten.
...
if (dimensionFloorExists && noEvidencePromiseExists) {
  ...
  if (!registered || registered.status !== 'PENDING_HUMAN_AUTHORITY') issue('UNREGISTERED_PROFESSIONAL_CONFLICT', 'Unevidenced 2/5 dimension floor conflicts with the no-invention promise and is not registered for human authority.'); else warning('PENDING_HUMAN_AUTHORITY', registered.conflict);
}
...
if (issues.length) {
  ...
  if (warnings.length) { console.error('\nContained pending conflicts:'); for (const item of warnings) console.error(`- [${item.code}] ${item.message}`); }
  process.exit(1);
}
...
console.log('Representation, state interpretation, source-of-truth, unresolved-state, privacy, authority, identity, telemetry, derived-state, data-path, aggregation and documentation boundaries are aligned.');
