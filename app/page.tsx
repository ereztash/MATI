...
import {
  ...
  rankDimensions, rubricForNextYear, scoreDimensions, selfEffectivenessAverage, smartGoalLooksValid, resolveStage, Stage, stageFromDate, stageLockReason, stageNames,
...
export default function Home() {
  ...
  function updatePlan(key: keyof MatiState['plan'], value: string) { setState((s) => ({ ...s, plan: { ...s.plan, [key]: value } })); }
  function updateContext(key: keyof MatiState['formative']['context'], value: string) { setState((s) => ({ ...s, formative: { ...s.formative, context: { ...s.formative.context, [key]: value } } })); }
  function updateAnswer(section: keyof FormativeAnswers, key: string, value: unknown) { setState((s) => ({ ...s, formative: { ...s.formative, answers: { ...s.formative.answers, [section]: { ...(s.formative.answers[section] as any), [key]: value } } } })); }
...
function FormativeMode({ state, updateContext, updateAnswer, updatePost, setState, saveFormative, dimensions, profile, previousFormative, showAnalysis }: { state: MatiState; updateContext: (key: keyof MatiState['formative']['context'], value: string) => void; updateAnswer: (section: keyof FormativeAnswers, key: string, value: unknown) => void; updatePost: (key: keyof MatiState['formative']['post'], value: string) => void; setState: React.Dispatch<React.SetStateAction<MatiState>>; saveFormative: () => void; dimensions: ReturnType<typeof scoreDimensions>; profile: ReturnType<typeof analyzeInteraction>; previousFormative?: string; showAnalysis: boolean; }) {
  ...
  return <><div className="sectionHead"><div><span className="kicker">שלב 2 · הערכה מעצבת</span><h1>מה באמת השתנה עד עכשיו?</h1></div><p>יש מסלול מלא ומסלול ממוקד. שניהם תקפים. מספיק אומדן קרוב — זו עצירה מקצועית לחשיבה, לא מבחן.</p></div>
    ...
    <div className="routeLabel">מה מתאים לך עכשיו?</div><div className="routePicker"><button aria-pressed={state.formative.route === 'short'} className={state.formative.route === 'short' ? 'choice activeChoice' : 'choice'} onClick={() => setState((s) => ({ ...s, formative: { ...s.formative, route: 'short' } }))}><span className="choiceDot" /><span><b>מסלול ממוקד</b><small>סעיפים 1, 2, 5, 8, 9 · כ־10 דקות</small></span></button><button aria-pressed={state.formative.route === 'full'} className={state.formative.route === 'full' ? 'choice activeChoice' : 'choice'} onClick={() => setState((s) => ({ ...s, formative: { ...s.formative, route: 'full' } }))}><span className="choiceDot" /><span><b>מסלול מלא</b><small>כל 9 הסעיפים · תמונה מערכתית רחבה</small></span></button></div><div className="softMeasure">גם מענה חלקי הוא בעל ערך. כשאין מספר מדויק, בחרי טווח קרוב או תיאור מילולי.</div>
...
function PersonalGanttView({ state, setState }: { state: MatiState; setState: React.Dispatch<React.SetStateAction<MatiState>> }) {
  ...
  const { start, end, now, milestones, cadence, planWindow, planWindowStatus } = gantt;
  ...
  return <section className="personalGantt"><div className="sectionHead compact"><div><span className="kicker">לוח הזמנים שלך</span><h3>הגאנט האישי שנגזר מהתוכנית</h3></div><p>{planWindow ? 'נבנה ממסגרת הזמן שכתבת ומהשדות שמילאת למעלה. נקודות הדרך האישיות ממוקמות בתוך מסגרת הזמן שלך.' : 'נבנה מתאריך שמירת התוכנית ומהשדות שמילאת למעלה.'} שדה ריק לא מקבל נקודת דרך. התאריך של כל נקודת דרך אישית הוא הצעה שאפשר לכוונן בכל שורה למטה — לא עובדה קבועה, בשונה משני חלונות ההערכה.</p></div>
    ...
    <div className="ganttTrack" role="img" aria-label={`ציר זמן מ־${fmt(start)} עד ${fmt(end)}${planWindow ? `, מסגרת הזמן שלך: ${fmt(planWindow.start)} עד ${fmt(planWindow.end)}` : ''}${cadence ? `, קצב מפגשים שזוהה: ${cadence.label}` : ''}`}>
      <div className="ganttBar">
        ...
        {bands.map((m) => <div key={m.kind} className={m.band === 'outline' ? 'ganttBand planWindowBand' : 'ganttBand'} aria-hidden="true" style={{ right: `${timelinePercent(m.date, start, end)}%`, width: `${Math.max(2, timelinePercent(m.rangeEnd!, start, end) - timelinePercent(m.date, start, end))}%` }} />)}
    ...
    {planWindowStatus === 'none' && <p className="ganttWindowNote">לא זוהו חודשים ב"מסגרת זמן" למעלה, אז נקודות הדרך ממוקמות על פני שנת התוכנית כולה. אם תכתבי שם טווח חודשים — למשל <strong>"ספטמבר–ינואר"</strong> — הן ימוקמו בתוכו.</p>}
    {planWindowStatus === 'closed' && <p className="ganttWindowNote">מסגרת הזמן שכתבת מתארת תקופה שכבר הסתיימה, אז נקודות הדרך ממוקמות על פני שנת התוכנית כולה. אם התהליך ממשיך — כדאי לעדכן את "מסגרת זמן" למעלה לתקופה שלפנייך.</p>}
    ...
    <ul className="ganttLegend">{milestones.map((m) => {
      const row = <span className="ganttLegendRow"><b aria-hidden="true" className={m.band === 'outline' ? 'ganttDot window' : m.rangeEnd ? 'ganttDot band' : 'ganttDot'} /><span><strong>{m.label}</strong> · {fmt(m.date)}{m.rangeEnd ? `–${fmt(m.rangeEnd)}` : ''}</span><small>{m.detail}</small></span>;
