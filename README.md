...
- בדיקת SMART מייעצת (לא חוסמת שמירה) מזהה אילו מהקריטריונים ספציפית / מדידה / רלוונטית / תחומה־בזמן ניכרים במשפט המטרה, עם רמז ממוקד לכל קריטריון חסר. בת־השגה מוצגת תמיד כשאלה רפלקטיבית קבועה ולא כ״זוהתה״ — שום רשימת מילים לא יכולה לדעת אם מטרה ריאלית.
- גאנט אישי נגזר מהתוכנית השמורה בלי לשאול שאלה נוספת: התחלה מתאריך השמירה, סוף בסגירת אותה שנת עבודה, ונקודות דרך מהשדות שכבר מולאו (צעד קטן, שיחת מנהלים, בדיקת גמישות) — שדה ריק פשוט לא מייצר נקודת דרך. שני חלונות ההערכה הקבועים מופיעים תמיד, ואינם ניתנים לשינוי.
- קצב מפגשים מזוהה אוטומטית מתוך ניסוח חופשי בשדה מסגרת הזמן (למשל ״אחת לשבועיים״) ומוצג כטקסטורה על הגאנט ובמשפט טקסט מפורש — בלי לשאול שאלה נוספת, ובלי לנחש כשאין ניסוח ברור או שיש סתירה.
- שלוש נקודות הדרך האישיות בגאנט ניתנות לכוונון (±1/±7 ימים) ישירות משורת ה־legend, עם איפוס לתאריך המוצע.
...
## מוכנות לפיילוט — מה חוסם

היעד: **מתי״א רג״ב, שנה״ל 2026-27.** חלון התכנון כבר פתוח.

ההגדרה המלאה של "מוכן" נמצאת ב־[`docs/MARKET_READINESS.md`](docs/MARKET_READINESS.md), והיא נגזרת מתשובות מנהלת המתי״א ([`docs/manager-decisions.md`](docs/manager-decisions.md)) ולא מרשימת פיצ'רים. בלשונה: כישלון הוא "האפליקציה עובדת טכנית מצוין" אבל "אין שינוי בהחלטות או בפעולות"; הצלחה היא מדריכה שאומרת "עשיתי משהו אחרת בעקבות זה".

שלושת החסמים הפתוחים לשלב 1 (ספטמבר):

- **אין מסלול שחזור למדריכה.** המידע ב־`localStorage` בלבד: ניקוי דפדפן או מעבר מכשיר = אובדן מלא, בלי דרך לשחזר. אומת בבדיקת cold-start ב־18.8.26.
- **שינויים בתוכנית לא נשמרים כ־before/after.** `updatePlan` דורס במקום. לכן אי אפשר להראות שמדריכה *שינתה* משהו — וזה בדיוק מדד ההצלחה שהמנהלת הגדירה (שאלות 2 ו־4).
- **לא נאמר במפורש שהרפלקציה אינה משמשת לדירוג.** לפי שאלה 20 זה התנאי שמאפשר מענה כן, כלומר תנאי מקדים לכל השאר.

`npm run check:readiness` מדווח על השורות הניתנות לבדיקה אוטומטית, ונכשל אם המסמך מצהיר שמשהו הושלם והקוד לא תומך בזה.

## פרטיות

בגרסת הפיילוט אין backend למסד נתונים ואין שליחת תוכן רפלקטיבי למודל AI חיצוני. המידע נשמר ב־`localStorage` בדפדפן המקומי, וניתן למחיקה מהממשק. לכן אין סנכרון בין מכשירים ומחיקת נתוני האתר תמחק גם את היסטוריית העבודה המקומית — ראו החסם הראשון למעלה: זהו סיכון אימוץ פתוח, לא רק מגבלה טכנית.
...
├── smart-criteria.ts              # advisory SMART-letter reader for the Stage 1 goal field
├── plan-timeline.ts               # personalized Gantt: milestones, fixed windows, adjustable dates
├── cadence.ts                     # conservative Hebrew cadence-phrase reader for the timeframe field
...
npm run check:readiness   # סטטוס מול ה־DoD; נכשל אם המסמך מקדים את הקוד
```

שלוש בדיקות החוזה רצות ב־CI לפני הבנייה. `check:readiness` הוא כלי ניווט ולא שער CI — הוא אמור לדווח על שורות פתוחות בלי להכשיל את הבנייה, ונכשל רק כשה־DoD מצהיר על משהו שהקוד לא מקיים.
