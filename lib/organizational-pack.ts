...
export interface OrganizationalPatternSummary {
  ...
  concerns: number;
  neutral: number;
  classification: ReturnType<typeof classifySystemicPattern> | null;
}
...
const CATEGORICAL_VALUES: Partial<Record<OrganizationalSignalKey, readonly unknown[]>> = {
  goal_attainment: ['none', 'partial', 'mostly', 'full'], meeting_execution: ['under70', '70-90', '90-100'],
  implementation_depth: ['consistent', 'shallow', 'partial'], student_impact: ['none', 'low', 'medium', 'high'],
  manager_commitment: ['high', 'medium', 'low', 'resistance'], resource_allocation: ['yes', 'partial', 'no'],
  teacher_independence: ['none', 'partial', 'most', 'all'], sustainability: ['yes', 'partial', 'no'], team_feedback_presence: [true, false],
};
const PERCENT_KEYS = new Set<OrganizationalSignalKey>(['implementation_rate', 'student_improvement_rate', 'manager_meeting_rate', 'resource_allocation_rate']);
...
function validSignalValue(key: OrganizationalSignalKey, value: unknown) {
  ...
  return Boolean(CATEGORICAL_VALUES[key]?.includes(value));
}
...
export function createOrganizationalPack(args: { contributorId: string; contextId: string; periodId: string; signals: OrganizationalSignal[] }): OrganizationalPack {
  return {
    ...
    contributorId: args.contributorId.trim(), contextId: args.contextId.trim(), periodId: args.periodId.trim(),
    signals: args.signals.map((signal) => ({ key: signal.key, stage: signal.stage, value: signal.value, confidence: signal.confidence, projection: 'aggregate_only', operationalImpact: signal.operationalImpact })),
  };
}
...
export function validateOrganizationalPack(input: unknown): input is OrganizationalPack {
  ...
  if (!exactKeys(raw, TOP_LEVEL_KEYS) || raw.schema !== ORGANIZATIONAL_PACK_SCHEMA) return false;
  ...
  return raw.signals.every((signal) => {
    ...
    if (!exactKeys(s, SIGNAL_KEYS) || typeof s.key !== 'string' || !SIGNAL_KEY_SET.has(s.key as OrganizationalSignalKey) || seen.has(s.key)) return false;
    seen.add(s.key);
    const key = s.key as OrganizationalSignalKey;
    return s.stage === 2 && validSignalValue(key, s.value) && (s.confidence === 'high' || s.confidence === 'medium') && s.projection === 'aggregate_only' && (s.operationalImpact === 'low' || s.operationalImpact === 'medium' || s.operationalImpact === 'high');
  });
}
...
export function summarizeOrganizationalPacks(packs: OrganizationalPack[]): OrganizationalPatternSummary[] {
  ...
  return keys.map((key) => {
    ...
    const observations = classifiable.map(({ pack, signal, concern }) => ({ key, contributorId: pack.contributorId, contextId: pack.contextId, periodId: pack.periodId, adverse: concern === true, operationalImpact: signal.operationalImpact }));
    return {
      key,
      ...
      classification: classifiable.length ? classifySystemicPattern(observations) : null,
    };
  });
}
